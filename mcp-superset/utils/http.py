...
def _auth_headers(ctx: Context) -> Optional[Dict[str, str]]:
    """Build auth headers for a Superset call from the caller's credential.

    The only credential used is the caller's Flask ``session`` cookie, which
    every Superset web login (including Azure AD / OAuth) issues. Returns None
    when the caller presented no session (unauthenticated).
    """
...
async def get_csrf_token(ctx: Context) -> Optional[str]:
    ...
    caller's own session cookie on this request too.
    """
...
async def make_api_request(
    ...
    The caller's forwarded Flask ``session`` cookie is the only credential used.
    A 401 is surfaced to the caller to re-authenticate; the server never falls
