"""
MCP protocol handlers — forward requests to upstream trino-mcp.
"""
...
async def forward_to_upstream(body: bytes, headers: dict) -> httpx.Response | None:
    ...
    try:
        ...
        # Extra: log SQL query if this is an execute_query or explain_query call
    ...
    # Auto-reinitialize on invalid session, then retry once
...
async def handle_tools_list(msg: dict, headers: dict) -> dict:
    ...
    tools = _cached_tools or FALLBACK_TOOLS
...
async def handle_tools_call(msg: dict, headers: dict) -> dict:
    ...
    _sep(f"handle_tools_call  tool={tool_name}")
    log.info("[tool-call] id=%s  session=%s", msg.get("id"), headers.get("mcp-session-id") or token_state.get("upstream_session_id"))
    log.info("[tool-call] tool=%s", tool_name)

    # Log query arg explicitly for SQL tools
    if tool_name in ("execute_query", "explain_query"):
        sql = tool_args.get("query", "")
    ...
    # Token exists but expired — try refresh then forward
    ...
    if await refresh_token():
        ...
        if resp and resp.status_code == 200:
            try:
                ...
                log.error("[tool-call] ← JSON parse error after refresh: %s", exc)
